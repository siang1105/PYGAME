...
def draw_init():
    ...
    draw_text(screen, '<- -> move spaceship, blank key fire bullet', 22, WIDTH/2, HEIGHT/2)
    draw_text(screen, 'tap any key to start ! ', 18, WIDTH/2, HEIGHT*3/4)
...
class Explosion(pygame.sprite.Sprite):
    def __init__(self, center, size):
        ...
        self.last_update = pygame.time.get_ticks() #回傳從初始化開始紀錄的毫秒數
        self.frame_rate = 50
...
while running:
    ...
    #判斷石頭 飛船相撞
    hits = pygame.sprite.spritecollide(player, rocks, True, pygame.sprite.collide_circle) #true代表石頭撞到後石頭要刪掉
    for hit in hits:
        new_rock()
    ...
    #判斷寶物 飛船相撞
    hits = pygame.sprite.spritecollide(player, powers, True) #true代表石頭撞到後石頭要刪掉
    for hit in hits:
        if hit.type == 'shield':
            shield_sound.play()
            player.health += 20
